...
class AETrainer(BaseTrainer):
    ...
    def train(self, dataset: BaseADDataset, ae_net: BaseNet):
        ...
        for epoch in range(self.n_epochs):
            ...
            # Test AUC.
            auroc, auprc, (val_loss_pos, val_loss_neg) = self.test(dataset, ae_net, verbose=False)
            ...
            self.writer.add_scalar('ae/AUROC', auroc, epoch)
            ...
            logger.info('  Epoch {}/{}\t Time: {:.3f}\t Loss: {:.8f}\t AUROC: {:.8f}'
                        .format(epoch + 1, self.n_epochs, epoch_train_time, avg_loss, auroc))
    ...
    def test(self, dataset: BaseADDataset, ae_net: BaseNet, verbose=True):
        ...
        if verbose:
            roc_x, roc_y, roc_thres = roc_curve(labels, scores)
            precision, recall, prc_thres = precision_recall_curve(labels, scores)
            thres_best = computeMaxYoudensIndex(roc_x, roc_y, roc_thres)
            self.thres = -thres_best
            logger.info('Test set AUROC: {:.2f}%'.format(100. * auroc))
            logger.info('Test set AUPRC: {:.2f}%'.format(100. * auprc))

            test_time = time.time() - start_time
            logger.info('Autoencoder testing time: %.3f' % test_time)
            logger.info('Finished testing autoencoder.')

        return auroc, auprc, (loss_pos, loss_neg)
