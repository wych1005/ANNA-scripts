...
class AnomalyDetectionTrainer(BaseTrainer):
    ...
    def train(self, dataset: BaseADDataset, net: BaseNet, augment: bool):
        ...
        for epoch in range(self.n_epochs):
            ...
            if self.test_auroc > best_auroc:
                best_auroc = self.test_auroc
                self.best_weights = {}
                for key in net.state_dict():
                    self.best_weights[key] = net.state_dict()[key].cpu()
                self.best_R = self.R.detach().clone()

            if self.test_auprc > best_auprc:
                best_auprc = self.test_auprc
                self.best_weights = {}
                for key in net.state_dict():
                    self.best_weights[key] = net.state_dict()[key].cpu()
                self.best_R = self.R.detach().clone()
            ...
            self.writer.add_scalar('anomaly/auroc', self.test_auroc, epoch)
            ...
            logger.info('  Epoch {}/{}\t Time: {:.3f}\t Loss: {:.8f}\t AUROC: {:.8f}'
                        .format(epoch + 1, self.n_epochs, epoch_train_time, avg_loss, self.test_auroc))
            logger.info('  Epoch {}/{}\t Time: {:.3f}\t Loss: {:.8f}\t AUPRC: {:.8f}'
                        .format(epoch + 1, self.n_epochs, epoch_train_time, avg_loss, self.test_auprc))
    ...
    def test(self, dataset: BaseADDataset, net: BaseNet, verbose=True):
        ...
        self.test_auroc = roc_auc_score(labels, scores)
        ...
        # print("SCORES: ", scores)
        logger.info(str(("SCORES: ", scores)))
        # print("LABELS: ", labels)
        logger.info(str(("LABELS: ", labels)))
        self.test_auprc = average_precision_score(labels, scores)

        if verbose:
            logger.info('Test set AUROC: {:.2f}%'.format(100. * self.test_auroc))
            logger.info('Test set AUPRC: {:.2f}%'.format(100. * self.test_auprc))
            ...
            preds[abs(preds) < abs(self.thres)] = 0
            preds[abs(preds) >= abs(self.thres)] = 1
           
            logger.info(str(("PREDS: ", preds)))


            fig, ax = plt.subplots()
            ax.set(xlabel='FPR', ylabel='TPR', title='ROC curve: {:.2f}% AUROC'.format(100.*self.test_auroc))
            ax.plot(roc_x, roc_y)
            ax.grid()
            self.roc_plt = (fig, ax)
            # plt.show()
            ...
            lw=2, label='PR curve (AUC = %0.2f)' % self.test_auprc)
            plt.plot([0, 1], [0.5, 0.5], color='navy', lw=lw, linestyle='--', label="No-skill classifier")
            plt.xlim([0.0, 1.0])
            plt.ylim([0.0, 1.05])
            plt.xlabel('Recall ')
            ...
            logger.info(str(("CM: ", cm)))
            sns.heatmap(cm , annot=True, fmt='.2%')
            plt.savefig("cm_anna_norm.png")
